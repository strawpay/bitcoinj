 ...
 *
 * <p>To begin, the client calls {@link PaymentChannelClientState#initiate(KeyParameter, ClientChannelModifier)}, which moves the channel into state
...
public abstract class PaymentChannelClientState {
     ...
     * {@link PaymentChannelClientState#initiate(KeyParameter, ClientChannelModifier)} after construction (to avoid creating objects for channels which are
     * not going to finish opening) and thus some parameters provided here are only used in
     * {@link PaymentChannelClientState#initiate(KeyParameter, ClientChannelModifier)} to create the Multisig contract and refund transaction.
     *
     ...
     * @param clientChannelModifier Modify the channel's configuration.
     *
     * @throws ValueOutOfRangeException   if the value being used is too small to be accepted by the network
     * @throws InsufficientMoneyException if the wallet doesn't contain enough balance to initiate
     */
    public abstract void initiate(@Nullable KeyParameter userKey, ClientChannelModifier clientChannelModifier) throws ValueOutOfRangeException, InsufficientMoneyException;
     ...
     * the channel properly. May only be called after {@link PaymentChannelClientState#initiate(KeyParameter, ClientChannelModifier)}
     */
