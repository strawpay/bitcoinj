...
public class PaymentChannelClient implements IPaymentChannelClient {
    ...
    private CloseReason receiveInitiate(Protos.Initiate initiate, Coin contractValue, Protos.Error.Builder errorBuilder) throws VerificationException, InsufficientMoneyException {
        ...
        if (wallet.isEncrypted() && (this.userKeySetup == null || userKeySetup.getKey().length == 0) )
            throw new IllegalArgumentException("Cannot initiate without a key parameter when the wallet is encrypted.");
    ...
    public ListenableFuture<PaymentIncrementAck> incrementPayment(Coin size, @Nullable ByteString info, @Nullable KeyParameter userKey) throws ValueOutOfRangeException, IllegalStateException {
        ...
        try {
            ...
            if (wallet.isEncrypted() && (userKey == null || userKey.getKey().length == 0) )
                throw new IllegalArgumentException("Must supply a key parameter when the wallet is encrypted.");
